...
import { deleteOwnPeripheralComment, updatePeripheralComment } from "@/lib/server/repositories/peripheral-comments-repository"
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string; commentId: string }> }) {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit({
      action: "peripheral_comment_edit",
      identifier: user.id,
      maxAttempts: 30,
      windowSeconds: 3600,
    })
    ...
    if (parsed.data.mentionedUserIds !== undefined) {
      ...
      const mentionedProfiles = requestedMentionIds.length ? await getUserProfiles(requestedMentionIds) : {}
      mentionedUserIds = requestedMentionIds.filter((id) => mentionedProfiles[id])
    }

    const result = await updatePeripheralComment({
      commentId,
      userId: user.id,
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string; commentId: string }> }) {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit({
      action: "peripheral_comment_delete",
      identifier: user.id,
      maxAttempts: 30,
      windowSeconds: 3600,
    })
    ...
    const result = await deleteOwnPeripheralComment({ commentId, userId: user.id })
    if (!result.ok) {
      return NextResponse.json({ error: result.error }, { status: result.status })
    }
    return NextResponse.json({ ok: true })

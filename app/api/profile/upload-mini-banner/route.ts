...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await importProfileMediaFromKlipy("mini-banner", authData.user.id, body.sourceUrl)
    if (!result.ok) {
      return NextResponse.json({ error: result.error }, { status: result.status })
    }
    ...
    console.error("[upload-mini-banner] falha ao importar GIF:", err)
    return NextResponse.json({ error: "Erro ao salvar o GIF do fundo do Mini Perfil." }, { status: 500 })
  }
}

...
const peripheralUpdate = z
  .object({
    ...
    category: z
      .enum(ALLOWED_PERIPHERAL_CATEGORIES, {
...
export async function PATCH(
  ...
  if (!hasAdminPermission(auth.profile, "peripherals_write")) {
    return NextResponse.json(
      { error: "Sem permissão para editar periféricos." },
      { status: 403 }
    )
  }
  ...
  if (parsed.data.tags !== undefined && current) {
    ...
    updateData.tags = sanitizeTagsForCategory(
      effectiveCategory,
      parsed.data.tags
    )
  }
  ...
  if (current) {
    ...
    const oldRanking = getRankingFromSpecs(
    ...
    if (oldCategory === newCategory) {
      ...
      if (oldRanking !== null)
        await cascadeRerank(db, oldCategory, id, oldRanking, null)
      if (newRanking !== null)
        await cascadeRerank(db, newCategory, id, null, newRanking)
    }
  }
...
export async function DELETE(
  ...
  if (!hasAdminPermission(auth.profile, "peripherals_write")) {
    return NextResponse.json(
      { error: "Sem permissão para deletar periféricos." },
      { status: 403 }
    )
  }
  ...
  revalidatePeripheral(current as { id: string; name: string } | null)

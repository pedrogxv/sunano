...
/** Lista paginada dos comentários de um periférico (`?page=1&sort=recent|aura`). */
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [result, totalCount] = await Promise.all([
      listPeripheralComments(id, { page, sort }),
      countPeripheralComments(id),
    ])
    return NextResponse.json({ ok: true, ...result, totalCount })
  } catch {
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit({
      action: "peripheral_comment_create",
      identifier: user.id,
      maxAttempts: 20,
      windowSeconds: 3600,
    })
    ...
    const result = await addPeripheralComment({
      peripheralId: id,
      userId: user.id,
      authorName,

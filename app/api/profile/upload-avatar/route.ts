...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await importProfileMediaFromKlipy("avatar", authData.user.id, body.sourceUrl)
    if (!result.ok) {
      return NextResponse.json({ error: result.error }, { status: result.status })
    }
    ...
    console.error("[upload-avatar] falha ao importar GIF:", err)
    return NextResponse.json({ error: "Erro ao salvar o GIF do avatar." }, { status: 500 })
  }
}

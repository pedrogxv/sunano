...
export function BlogPostContent({ post }: { post: BlogPost | null }) {
  ...
  // Defesa em profundidade: só renderiza o link cru de fallback se for
  // http(s) — a API já valida isso ao salvar, mas isso protege posts
  // gravados antes da validação existir.
        ...
        <CardContent className="space-y-6 p-6 md:p-7">
          {embedUrl ? (
            ...
            <div className="rounded-xl border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
              {t.blog.externalVideo} <a className="text-primary underline" href={safeVideoUrl} target="_blank" rel="noreferrer">{safeVideoUrl}</a>
            </div>
          ) : null}
